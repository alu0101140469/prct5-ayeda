 ...
 */
template <class Key>
inline void selectionSort(staticSequence<Key>& seq, const unsigned size, const int log_level) {
  ...
  for (unsigned i = 0; i + 1 < size; ++i) {
    ...
    if (log_level >= 5) printSequence(seq, size); // Mostrar la secuencia después de cada paso si se activa
  }
}
 ...
 */
template <class Key>
inline void bubbleSort(staticSequence<Key>& seq, const unsigned size, const int log_level) {
  if (size < 2) return; // No es necesario ordenar secuencias de 0 o 1 elementos
 ...
 */
template <class Key>
inline void mergeSortRec(staticSequence<Key>& seq, const unsigned ini, const unsigned fin, const int log_level) {
  ...
  mergeSortRec(seq, ini, mid, log_level); // Ordenar recursivamente el primer rango
  mergeSortRec(seq, mid, fin, log_level); // Ordenar recursivamente el segundo rango
  mergeRanges(seq, ini, mid, fin); // Fusionar los dos rangos ordenados
  if (log_level >= 5) printSequence(seq, seq.size()); // Mostrar la secuencia después de cada paso si se activa
}
 ...
 */
template <class Key>
inline void mergeSort(staticSequence<Key>& seq, const unsigned size, const int log_level) {
  if (size < 2) return; // No es necesario ordenar secuencias de 0 o 1 elementos
  mergeSortRec(seq, 0, size, log_level); // Iniciar la parte recursiva de MergeSort con el rango completo de la secuencia
}
 ...
 */
template <class Key>
inline void heapSort(staticSequence<Key>& seq, const unsigned size, const int log_level) {
  if (size < 2) return; // No es necesario ordenar secuencias de 0 o 1 elementos
 ...
 */
template <class Key>
inline void decreasingIncrementSort(staticSequence<Key>& seq, const unsigned size, const double alpha, const int log_level) {
  if (size < 2) return; // No es necesario ordenar secuencias de 0 o 1 elementos
